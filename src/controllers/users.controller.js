...
export const fetchAllUsers = async (req, res, next) => {
    try {
        ...
        res.json({
            ...
            users: allUsers.map(u => ({ id: u.id, name: u.name, email: u.email, role: u.role, created_at: u.created_at, updated_at: u.updated_at })),
...
export const updateUser = async (req, res, next) => {
    try {
        ...
        if (!idValidation.success) {
            return res.status(400).json({ error: 'Validation failed', details: idValidation.error.issues });
        }
        const bodyValidation = updateUserSchema.safeParse(req.body);
        if (!bodyValidation.success) {
            return res.status(400).json({ error: 'Validation failed', details: bodyValidation.error.issues });
        }
        ...
        logger.info(`Updating user id=${id} by user id=${decoded.id} role=${decoded.role}`);
        const updated = await updateUserService(id, updates);

        res.json({
            message: 'User updated successfully',
            user: { id: updated.id, name: updated.name, email: updated.email, role: updated.role, created_at: updated.created_at, updated_at: updated.updated_at },
        });
    } catch (e) {
...
export const deleteUser = async (req, res, next) => {
    try {
        ...
        logger.info(`Deleting user id=${id} by user id=${decoded.id} role=${decoded.role}`);
